 ...
 * @param intro 소개글
 * @param timestamp 작성 시간
 * @param text 입력한 텍스트 (빈 문자열이면 isSilenced 처리)
 * @param tag 태그된 이름들 (배열) (빈 배열이거나 null이면 !isNotTagged 처리)
 * @param isMine 내 피드인 경우 (기본값 false => true일 때만 따로 표시)
 * @param isLiked 좋아요를 누른 상태인지 (기본값 false => true일 때만 따로 표시)
 * @param onLikeToggle 좋아요 토글 함수
 * @param onShare 공유 함수
 * @param isMyName 이름이 내 이름인지 확인하는 함수
 * @param onRequest 태그 요청 함수
 * @param onPoke 콕 찌르기 함수
 * @param onAdd 태그 수정 함수
 */
export default function Feed({
  ...
  return (
    <S.Container>
      ...
      <S.TextContainer $isSilenced={isSilenced}>
      ...
      <S.TagContainer>
        <S.TagBox>
          ...
          {tag && (
            <S.Tag>
              {tag.map((name, index) => (
                <S.Name $isMyName={isMyName(name)} key={index}>
                  {name}
                </S.Name>
              ))}
              <S.PlusBtn type="button" onClick={isMine ? onAdd : onRequest}>
                <svg
                  xmlns="http://www.w3.org/2000/svg"
                  width="8"
                  height="8"
          ...
          {isSilenced && !isMine && (
            <S.PokeBtn type="button" onClick={onPoke}>
              콕 찌르기
            </S.PokeBtn>
          )}
          {!isSilenced && isNotTagged && !isMine && (
            <S.RequestBtn type="button" onClick={onRequest}>
              태그 요청하기
            </S.RequestBtn>
          )}
          {!isSilenced && isNotTagged && isMine && (
            <S.PlusBtn type="button" onClick={onAdd}>
              <svg
                xmlns="http://www.w3.org/2000/svg"
                width="8"
                height="8"
